...
### v3.0.0 (2026-04-18)
- 🛑 **优化关闭服务**：移除多余弹窗，一点即关
- 🚀 **一键启动脚本**：双击运行 bat 即可启动
- 🔧 **环境检测脚本**：自动检测/安装 Python、ffmpeg 依赖

### v2.0.0 (2026-04-17)
- ✨ **新增 MiniMax AI 润色**：自动为转录文本添加标点、生成视频大纲
- 🔧 **API 测试接口**：可实时测试 MiniMax API 连接是否正常
- 📝 **润色后自动检查**：润色结果无标点时自动补充
- 🗑️ **移除打包功能**：简化程序结构
- 🧹 **清理优化**：启动时自动清理旧日志
...
双击 `检测环境.bat`，自动检测并安装缺失的依赖（Python、ffmpeg）。

> 提示：如果提示 ffmpeg 未安装，输入 `y` 自动安装，或手动运行 `winget install Gyan.FFmpeg`

### 3. 启动服务

双击 `双击运行(后等待几秒钟).bat`，等待几秒后浏览器自动打开。
...
双击 `双击运行(后等待几秒钟).bat`，等待 8 秒左右浏览器自动打开。

### 关闭服务

双击 `关闭服务.bat`，或点击页面上的「关闭服务」按钮。
